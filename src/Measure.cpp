//  Measure.cpp :: Measures and Music sequences
//  Copyright 2011 - 2013 Keigen Shu

#include <algorithm>
#include "Measure.hpp"

void Measure::sort_lists()
{
    lNotes .sort(cmpNote_Greater);
    lParams.sort(cmpParamEvent_Greater);
}

TTime Measure::getTimeFromTickCount(unsigned t) const {
    return TTime(t % beatSize, (t / beatSize) % beatCount, t / tickCount);
}

//# TODO Complete this table.
std::pair<double, std::pair<unsigned, unsigned>> TS_Lookup[] = {
...
void Measure::setTimeSignature(unsigned a, unsigned b) { Measure (a,b); }

void Measure::setTimeSignature(double z)
{
    for(auto p : TS_Lookup) {
        if (p.first == z) {
            Measure (p.second.first, p.second.second);
            beatCount = p.second.first;
            beatSize  = p.second.second;
            tickCount = beatCount * beatSize;
            return;
        }
    }
    printf("Measure [warn] Time signature lookup failed.\n");
    beatCount = 4;
    beatSize  = 48;
    tickCount = beatCount * beatSize;
    return;
